...
class PicoController(Node):
    """
    Independent PID controllers for Surge, Sway, Heave (Depth), and Yaw.
    Consumes error setpoints from /to_pico and feedback from /altimeter.
    """
    ...
    def __init__(self):
        ...
        self.declare_parameter('kp_sway', 1.5)
        self.declare_parameter('ki_sway', 0.0)
        self.declare_parameter('kd_sway', 0.3)
        ...
        # Output Saturation (N or N·m)
        self.declare_parameter('surge_limit', 40.0)
        self.declare_parameter('sway_limit', 40.0)
        ...
        # PID Accumulators
        self.i_surge = self.i_sway = self.i_depth = self.i_yaw = 0.0
        self.prev_e_surge = self.prev_e_sway = self.prev_e_depth = self.prev_e_yaw = 0.0
    ...
    def control_loop(self):
        ...
        effective_stop = self.stop_bit != 0 or age > timeout

        i_clamp = self.get_parameter('i_clamp').value

        # Depth Control (Always active if depth is received)
        ...
        if not effective_stop:
            ...
            u_sway, self.i_sway, self.prev_e_sway = self._pid(
                self.delta_s, self.i_sway, self.prev_e_sway,
            ...
            s_lim, sw_lim, y_lim = (self.get_parameter('surge_limit').value,
                                    self.get_parameter('sway_limit').value,
                                    self.get_parameter('yaw_limit').value)
                                    
            cmd.linear.x = float(max(-s_lim, min(s_lim, u_surge)))
            cmd.linear.y = float(max(-sw_lim, min(sw_lim, u_sway)))
            cmd.angular.z = float(max(-y_lim, min(y_lim, u_yaw)))
        else:
            self.i_surge = self.i_sway = self.i_yaw = 0.0
            self.prev_e_surge = self.prev_e_sway = self.prev_e_yaw = 0.0
