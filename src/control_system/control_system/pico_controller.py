...
class PicoController(Node):
    """
    Independent PIDs driven by /to_pico setpoints and /altimeter feedback.

    Inputs:
      /to_pico  (custom_interfaces/ToPico)
          delta_yaw     yaw error    (rad)   -> angular.z
          delta_d       surge error  (m)     -> linear.x
          delta_s       sway error   (m)     -> linear.y
    ...
    def __init__(self):
        ...
        # PID gains — surge / sway / depth / yaw
    ...
    def control_loop(self):
        ...
        if not effective_stop:
            ...
            sw_lim = self.get_parameter('sway_limit').value
