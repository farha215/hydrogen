...
| `data_distance_node` | `data_distance_node.py` | **ML + depth-fusion 3D detector**. Synchronises depth + RGB + 2D detections (YOLO), extracts z from depth ROI (median of finite pixels in bounding box), back-projects via camera intrinsics. Publishes `Detection3DArray` on `/detections_3d`. |
...
| YOLO confidence threshold | `prequalification_bt/bt_nodes.h` (`score >= 0.7` for gate, `0.45` for pole) |
